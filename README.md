...
- **节奏热力条**（替代旧跟打地图）：每字一格、绿→黄→橙→红渐变、最慢字白色发光 + 顶部刻度 + 慢字红色三角 ms 标记纵向堆叠
...
- **发文状态浮窗**：渐变卡片 / 霓虹进度条 / 三联字数大数 / 磁吸跟随主窗
- **历史表底部金色汇总行**：今日训练时长 / 段数 / 均速 / 击键 / 码长 + 累计均速
...
- **KeyHook 严格物理键过滤**（5 层）：LLKHF_INJECTED / LLKHF_LOWER_IL_INJECTED / scanCode==0 / VK_PROCESSKEY / 非法 vk，排除所有 IME / SendInput 模拟键，击键 / 码长 / 左右手统计准确
